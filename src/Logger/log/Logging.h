...
public:
    ...
    // member function
    Logger(const char* file, int line); //用于在代码中记录日志的位置。
    Logger(const char* file, int line, LogLevel level); //指定日志级别。
    Logger(const char* file, int line, LogLevel level, const char* func); //同时记录函数名。
    ~Logger();

    // 流是会改变的 返回一个可以写入日志信息的流
